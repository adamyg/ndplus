...
use NaturalDocs::Languages::Cpp;                #ND+, C++
...
sub ProcessProperties #(properties, version, tempExtensions, tempShebangStrings)
    ...
    for (my $i = 3; $i < scalar @$properties; $i += 3)
        ...
        elsif ($keyword eq 'tab length')                #ND+, tab&indent
            {  $language->SetTabLength($value);  }

        elsif ($keyword eq 'indent length')             #ND+, tabs&indent
            {  $language->SetIndentLength($value);  }
...
sub SaveFile #(isMain)
        ...
        while (my ($keyword, $value, $comment) = NaturalDocs::ConfigFile->GetLine())
            ...
            if ($keyword eq 'language')
                ...
                push @ignoredExtensions, split(/ /, $value);
                    ...
                    $keyword eq 'tab length' || $keyword eq 'indent length' )   #ND+, tab&indent
                {
    ...
    . "#\n"
    . "# Tab Length: [value]\n"                         #ND+, tabs&indent
    . "#    Defines the size of a tab in spaces, overriding the system global setting\n"
    . "#    configured via the command line -t option or its default value of 4.\n"
    . "#\n"
    . "# Indent Length: [value]\n"                      #ND+, tabs&indent
    . "#    Defines the size of an indentation in spaces which is used by the parser\n"
    . "#    when determining list and paragraph nesting within embedded documention.\n"
    . "#    A value of zero disables automatic indentation processing and the resulting\n"
    . "#    nesting of bullet, ordered and description list items. The default is the\n"
    . "#    tab length.\n"
    . "#\n"
    . "#------------------------------------------------------------------------------\n"
    ...
    for (my $i = 0; $i < scalar @segments; $i += 3)
        ...
        if (exists $properties->{'tab length'})         #ND+, tab&indent
            {
            print FH_LANGUAGES '   Tab Length: ' . $properties->{'tab length'} . "\n";
            };

        if (exists $properties->{'indent length'})      #ND+, tab&indent
            {
            print FH_LANGUAGES '   Indent Length: ' . $properties->{'indent length'} . "\n";
            };
    ...
    if (defined $languageName)
        ...
        if ($languageName eq 'shebang script')
            ...
            if (exists $shebangFiles{$sourceFile})
                ...
                return undef                            # ND+, footer
                    if ($sourceFile eq '__page__footer__' || $sourceFile eq "");
...
#
#       Returns the language associated with the specified type.
#
#   Parameters:
#
...
sub LanguageInstance #(type)
    {                                           #ND+
        my ($self, $type) = @_;

        if (exists $languages{$type})
            { return $languages{$type}; };
            
        my $languageName = $extensions{$type};
