    ...
    if (defined $options)
        ...
        foreach (@$options)
            ...
            if (/^title\s*=(.*)$/i)
                ...
                elsif ($format =~ /^nd$/i      || $format =~ /^nd\+$/i)
                    { $format = 1; }

                elsif ($format =~ /^basic$/i   || $format =~ /^nd\-$/i)
                    { $format = 2; }

                elsif ($format =~ /^simple$/i  || $format =~ /^wiki$/i)
                    { $format = 3; }

                elsif ($format =~ /^csv$/i     || $format =~ /^cvs$/i)
                    { $format = 4; }            # allow a common misspelling
                ...
                elsif ($justification eq 'centre' || $justification eq 'center')
                    { $justification = 1; }
...
sub ParseLine
    ...
    if (2 == $format)                           # 'ndbasic'
        { return $self->ParseLineNDBASIC($line, $break); }

    elsif (3 == $format)                        # 'simple'
...
sub ParseLineND #(line)
    ...
    # Otherwise field text
        ...
        # If the line starts with a row delimiter (single bang)

        elsif ($self->{FORMAT} >= 1 && $line =~ /^(\s*\!)\s+/)
            {
            $newrow = length($1);
            }
...
LOOP:   while ($pos < $length)
            ...
            if (length($text))
                ...
                $text =~ s/^(\s*\!)!(\s+)/$1 $2/;   # convert leading '!!' -> '! '
...
#
#   Function:       ParseLineNDBASIC
#       Parse 'ndbasic' table definition
#
sub ParseLineNDBASIC #(line)
    {
    my ($self, $line) = @_;
    ...
    if (0 == $self->{CURSOR} && $self->{HEADER})
        ...
        while ($line =~ /^(\s*[\[\|])([^\[\|\]]+)([\]]{0,1})/)
            {
            my ($leading, $text, $len, $trailing) = ($1, $2, length($2), $3);

            my $margin = length($leading);

            # Determine leading and rules length,
            #
            #   [] - The ruler includes the opening brace and the optional closing brace.
            #
            #   | -  The ruler length is the field contained between the delimiters.
            #

            $pos += $margin;                    # increment cursor past leading text
