...
sub ParseComment #(file, commentLines, isJavaDoc, lineNumber, parsedTopics)
    ...
    while ($index < scalar @$commentLines)
        ...
        if ($inCodeSection)
            {                                           #ND+, ditaa/mscgen/sdedit/plantuml
            if ($commentLines->[$index] =~ /^ *\( *(?:end|finish|done)(?: +(?:table|code|example|diagram|ditaa|mscgen|sdedit|plantuml))? *\)$/i)
                {  $inCodeSection = undef;  };
        ...
        # If we're on a normal content line within a topic
            ...
            $bodyEnd++;
                                                        #ND+, ditaa/mscgen/sdedit/plantuml
            if ($commentLines->[$index] =~ /^ *\( *(?:(?:start|begin)? +)(?:ditaa|mscgen|sdedit|plantuml)([^\)]*)\)$/i)
                {  $inCodeSection = 1;  }
                                                        #ND+, prototype
...
sub FormatBody #(file, commentLines, startingIndex, endingIndex, type, isList)
    ...
    use constant TAG_PLANTUML => 13;
    use constant TAG_ADMONITION => 14;
    use constant TAG_QUOTE => 15;

    my %lineEnders = (
            TAG_BULLETLIST() => '</li>',
            TAG_DESCRIPTIONLIST() => '</dd>',
            TAG_ORDEREDLIST() => '</li>');

    my %tagEnders = (
            TAG_NEW() => '',                            #ND+, ordered/ditaa/ededit/plantuml
            TAG_NONE() => '',
            TAG_PARAGRAPH() => '</p>',
            TAG_BULLETLIST() => '</ul>',
    ...
    while ($index < $endingIndex)
        ...
        # If we're in a tagged section, code, ditaa, mscgen, sdedit, plantuml or drawing
        elsif ($currTag == TAG_CODE || $currTag == TAG_QUOTE ||
                    $currTag == TAG_DITAA || $currTag == TAG_MSCGEN || $currTag == TAG_SDEDIT || 
                    $currTag == TAG_PLANTUML || $currTag == TAG_DRAWING)
            {                                           #ND+, ditaa/mscgen/sdedit/plantuml/drawing
            if ($commentLine =~ /^ *\( *(?:end|finish|done)(?: +(?:table|code|example|diagram|ditaa|mscgen|sdedit|plantuml|drawing))? *\)$/i)
                {
        ...
        elsif ($admonition && $commentLine =~ /^\s*\(end\!\)\s*$/i)
            {                                           #ND+, admonition's
            $output .= $admonition;
            $admonition = '';
            }
        ...
        # If we're not in a tagged section nor table...
            ...
            # If the line could be a admonishment container
                        ...
                        exists $admonitions{lc($1)} &&
                    (!$file || $file->Modeline('admon', 1)))
                {                                       #ND+, admonition's
                my $leading = $1;
                my $trailing = ":" . $2;
                ...
                $currTag = TAG_NONE;
                                                        ...
                                                        # captialise leading character
                $leading =~ s/([\w]+)/\u\L${leading}/g;
                $output .= $admonition;
                $output .=
                    '<admon-' . $leading . '>' .
            ...
            # If the line could be a sub-header
            elsif ($prevLineBlank && $commentLine =~ /^\s*(.*)([^ \t])::\s*$/)
                {                                       #ND+, subheader
                my $headerText = $1 . $2;
                ...
                $output .= $admonition;
                $output .= '<h2>' . $self->RichFormatTextBlock($headerText) . '</h2>';
                $admonition = '';
            ...
            # If the line looks like a line dita tag...
            ...
            # If the line looks like a line plantuml tag...
            elsif ($commentLine =~ /^ *\( *(?:(?:start|begin)? +)?plantuml([^\)]*)\)$/i)
                {                                       #ND+, plantuml
                my $options = $1;
                ...
                $output .= $indent->End($currTag) . "<plantuml $options>\n";
                $currTag = TAG_PLANTUML;
                }
            ...
            # If the line looks like an inline image...
                ...
                {
                my ($leading, $body, $trailing) = ($1, $2, $3);
                ...
                $output .= '<img mode="inline" target="' . NaturalDocs::NDMarkup->ConvertAmpChars($body) . '" '
                                . 'original="' . NaturalDocs::NDMarkup->ConvertAmpChars($leading . $body . $trailing) . '">';
    ...
    if ($table)
        ...
        {
        $codeBlock =~ s/\n+$//;
        if ($currTag == TAG_DITAA || $currTag == TAG_MSCGEN || $currTag == TAG_SDEDIT || 
                    $currTag == TAG_PLANTUML || $currTag == TAG_DRAWING)
            {  $output .= $codeBlock;  }                #ND+, ditaa/mscgen/sdedit/plantuml
        else
            {  $output .= NaturalDocs::NDMarkup->ConvertAmpChars ($codeBlock);  }
        $codeBlock = undef;
        }
...
sub RichFormatTextBlock #(text)
    ...
    # Find embedded image links.  Inline images should already be pulled out by now.

                                                #ND+, smart see references
    $text =~ s{(\( *see +)([^\)]+?)( *\))}{$self->InlineSee($1, $2, $3)}gse;
    ...
    while ($index < scalar @textBlocks)
        {
        if ($textBlocks[$index] eq "\x1E")              # quoted blocks
            {
            $output .= '<';
            $index++;
        ...
        elsif ($textBlocks[$index] eq '<' &&            #ND+, <link:'xxxx'>
                    $textBlocks[$index+1] eq 'link:' && $textBlocks[$index+2] eq '\'')
            {
            my $endIndex = $index + 3;
            my $linkText = '';

            while ($endIndex < (scalar @textBlocks - 1))
                {
                if ($textBlocks[$endIndex] eq '\'' && $textBlocks[$endIndex+1] eq '>')
                    {
...
#   Function: InlineSee
#       Parse inline image references.
#
#   Parameters:
#       leading - Leading text, for example "See "
#       body -  Reference body.
#       trailing - Trailing text, if any.
#
#   Returns:
#       The formatted text block using NDMARKUP.
#
sub InlineSee #(leading, body, trailing)
    {                                                   #ND+
    my ($self, $leading, $body, $trailing) = @_;
    my $isImage = NaturalDocs::Project->DetermineIfImage($body);

    if (! $isImage)
        {                                               # (See <reference>)
        return "(${leading} \x1E" . 'url target="' . NaturalDocs::NDMarkup->ConvertAmpChars($body) . '" '
                    . 'name="' . NaturalDocs::NDMarkup->ConvertAmpChars($body) . '"' . "\x1F" . ($trailing ? " ${trailing})" : ")");
        }

                                                        # image
    return "\x1E" . 'img mode="link" target="' . NaturalDocs::NDMarkup->ConvertAmpChars($body) . '" '
                . 'original="' . NaturalDocs::NDMarkup->ConvertAmpChars($leading . $body . $trailing) . '"' . "\x1F";
    }
