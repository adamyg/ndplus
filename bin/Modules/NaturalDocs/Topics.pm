...
sub LoadFile #(isMain)
    ...
    if ($version = NaturalDocs::ConfigFile->Open($file))
        ...
        while (my ($keyword, $value) = NaturalDocs::ConfigFile->GetLine())
            ...
            # the settings if $topicTypeObject is undef.
            ...
            elsif ($keyword eq 'sort groupings')        #ND+, sort
                {
                $value = lc($value);
                ...
                if ($value eq 'yes')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetSortGroupings(1);  };
                    }
                elsif ($value eq 'no')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetSortGroupings(0);  };
                    }
                else
                    {
                    NaturalDocs::ConfigFile->AddError('Set SortGroup lines can only be "yes" or "no".');
                    };
                }

            elsif ($keyword eq 'merge groupings')       #ND+, merge
                {
                $value = lc($value);
                ...
                if ($value eq 'yes')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetMergeGroupings(1);  };
                    }
                elsif ($value eq 'no')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetMergeGroupings(0);  };
                    }
                else
                    {
                    NaturalDocs::ConfigFile->AddError('Set MergeGroup lines can only be "yes" or "no".');
                    };
                }

            elsif ($keyword eq 'multiple')              #ND+, package
                {
                $value = lc($value);
                ...
                if ($value eq 'normal')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetMultiple(0);  };
                    }
                elsif ($value eq 'merge')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetMultiple(::MULTIPLE_MERGE());  };
                    }
                elsif ($value eq 'split')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetMultiple(::MULTIPLE_SPLIT());  };
                    }
                elsif ($value eq 'summaries')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetMultiple(::MULTIPLE_SUMMARIES());  };
                    }
                else
                    {
                    NaturalDocs::ConfigFile->AddError('Multiple lines can only be "normal", "summaries", "merge" or "split".');
                    };
                }

            elsif ($keyword eq 'splittable')            #ND+, package
                {
                $value = lc($value);
                ...
                if ($value eq 'yes')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetSplittable(1);  };
                    }
                elsif ($value eq 'no')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetSplittable(0);  };
                    }
                else
                    {
                    NaturalDocs::ConfigFile->AddError('Splittable can only be "yes" or "no".');
                    };
                }

            elsif ($keyword eq 'summaries')             #ND+, summaries
                {
                $value = lc($value);
                ...
                if ($value eq 'yes')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetDontSummaries(0);  };
                    }
                elsif ($value eq 'no')
                    {
                    if (defined $topicTypeObject)
                        {  $topicTypeObject->SetDontSummaries(1);  };
                    }
                else
                    {
                    NaturalDocs::ConfigFile->AddError('Set Summaries lines can only be "yes" or "no".');
                    };
                }
...
sub SaveFile #(isMain)
        ...
        while (($keyword, $value) = NaturalDocs::ConfigFile->GetLine())
            ...
            if ($keyword eq 'topic type' || $keyword eq 'alter topic type')
                ...
                $properties{$topicTypeName}->{$keyword} = $value;
                    ...
                    $keyword eq 'merge groupings' ||    #ND+, merge
                    $keyword eq 'sort groupings' ||     #ND+, sort
                    $keyword eq 'multiple' ||           #ND+, package
                    $keyword eq 'splittable' ||         #ND+, package
                    $keyword eq 'summaries'             #ND+, summaries
                    )
                {
    ...
    my @propertyOrder = (
            'Plural', 'Index', 'Scope', 'Class Hierarchy',
            'Page Title If First', 'Break Lists',
            'Merge Groupings', 'Sort Groupings',        #ND+, merge/sort
            'Multiple', 'Splittable',                   #ND+, package
            'Summaries');                               #ND+, summaries
