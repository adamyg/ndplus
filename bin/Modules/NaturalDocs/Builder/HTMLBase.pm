...
use FindBin qw($Bin);                           #ND+, plugins
use NaturalDocs::Builder::toSVG;
...
#   array: syntaxBrushes
#
#   Language to brush mapping
#
my @syntaxHiliteBrushes = (                     # ND+, syntax3 (TODO load at runtime)
        'shBrushAppleScript.js:'.   'applescript'                           ,
        'shBrushAS3.js:'.           'actionscript3,as3'                     ,
        'shBrushBash.js:'.          'bash,shell,sh,ksh,csh,shebangscript'   ,
        'shBrushColdFusion.js:'.    'coldfusion,cf'                         ,
        'shBrushCpp.js:'.           'cpp,c,c++,c/c++'                       ,
        ...
        'shBrushPerl.js:'.          'perl,pl,pm'                            ,
        'shBrushPhp.js:'.           'php'                                   ,
        'shBrushPlain.js:'.         'text,plain,textfile'                   ,
        'shBrushPython.js:'.        'py,python'                             ,
        'shBrushRuby.js:'.          'ruby,rails,ror,rb'                     ,
        'shBrushSass.js:'.          'sass,scs,s'                            ,
        'shBrushScala.js:'.         'scala'                                 ,
        'shBrushSql.js:'.           'sql'                                   ,
        'shBrushVb.js:'.            'vb,vbnet,visualbasic'                  ,
...
#
#   var: imageSyntaxBrushes
#
#   Syntaxhilite markup set
#
my %imageSyntaxBrushes;
...
sub EndBuild #(hasChanged)
    ...
    my $cssInDirectory = NaturalDocs::Settings->StyleDirectory();
    my $cssOutDirectory = $self->CSSDirectory();

    my $javaInDirectory = NaturalDocs::Settings->JavaScriptDirectory();
    my $javaOutDirectory = $self->JavaScriptDirectory();
    ...
    for (my $i = 0; $i < scalar @$styles; $i++)
        ...
        if (scalar @$styles == 1)
            ...
            {  $outputCSSFile = NaturalDocs::File->JoinPaths( $cssOutDirectory, ($i + 1) . '.css');  };


        my $masterCSSFile = NaturalDocs::File->JoinPaths( NaturalDocs::Settings->ProjectDirectory(), $styles->[$i] . '.css' );

        if (! -e $masterCSSFile)
            {  $masterCSSFile = NaturalDocs::File->JoinPaths( $cssInDirectory, $styles->[$i] . '.css' );  };
    ...
    for (;;)
        {
        my $file = NaturalDocs::File->JoinPaths($cssOutDirectory, $deleteFrom . '.css');
    ...
    # Import SyntaxHiliting
    #   style sheets and JavaScript             #ND+, syntax 3+

    opendir DIRECTORYHANDLE, $cssInDirectory;
    my @syntaxStyles = grep /^sh.*\.css/, readdir(DIRECTORYHANDLE);
    closedir DIRECTORYHANDLE;

    foreach (@syntaxStyles) {
        my $cssin = NaturalDocs::File->JoinPaths( $cssInDirectory, $_ );
        my $cssout = NaturalDocs::File->JoinPaths( $cssOutDirectory, $_ );

        if (! -e $cssout ||
                (stat($cssin))[9] != (stat($cssout))[9] || -s $cssin != -s $cssout)
            {
            NaturalDocs::File->Copy($cssin, $cssout);
            }
    }

    opendir DIRECTORYHANDLE, $javaInDirectory;
    my @syntaxJava = grep /^sh.*\.js/, readdir(DIRECTORYHANDLE);
    closedir DIRECTORYHANDLE;

    foreach (@syntaxJava) {
        my $jsin = NaturalDocs::File->JoinPaths( $javaInDirectory, $_ );
        my $jsout = NaturalDocs::File->JoinPaths( $javaOutDirectory, $_ );

        if (! -e $jsout ||
                (stat($jsin))[9] != (stat($jsout))[9] || -s $jsin != -s $jsout)
            {
            NaturalDocs::File->Copy($jsin, $jsout);
            }
    }

    # Update the JavaScript files

    my $jsMaster = NaturalDocs::File->JoinPaths( $javaInDirectory, 'NaturalDocs.js' );
    my $jsOutput = $self->MainJavaScriptFile();

    # We check both the date and the size in case the user switches between two styles which just happen to have the same
    # date.  Should rarely happen, but it might.
    if (! -e $jsOutput ||
            (stat($jsMaster))[9] != (stat($jsOutput))[9] || -s $jsMaster != -s $jsOutput)
        {
        NaturalDocs::File->Copy($jsMaster, $jsOutput);
        };
    my @indexes = keys %{NaturalDocs::Menu->Indexes()};

    open(FH_INDEXINFOJS, '>' . NaturalDocs::File->JoinPaths( $javaOutDirectory, 'searchdata.js'));
...
#
sub BuildMenuSegment #(outputDirectory, menuSegment, depth)
    {
    my ($self, $outputDirectory, $menuSegment, $depth) = @_;

    my $output;
    my $groupLength = 0;

    my $topLevel = ($depth == 1 ? 1 : 0);       #ND+, formatting

    foreach my $entry (@$menuSegment)
        {
        $output .= "\n"."\t"x$depth;            #ND+, formatting

        if ($entry->Type() == ::MENU_GROUP())
            {
            my ($entryOutput, $entryLength) =
                $self->BuildMenuSegment($outputDirectory, $entry->GroupContent(), undef, $depth+1);
...
sub BuildContent #(file)
    ...
    %imageSyntaxBrushes = ();                   #ND+, syntax

    my $imgSeq = 0;                             #ND+, image/ditaa
    ...
    while ($i < scalar @$parsedFile)
        ...
        my $anchor = $self->SymbolToHTMLSymbol($topic->Symbol());
        ...
        $output .=
            '<div class="C' . NaturalDocs::Topics->NameOfType($topic->Type(), 0, 1) . '">'
                    . '<div class=CTopic' . ($i == 0 ? ' id=MainTopic' : '') . '>'
                    . '<' . $headerType . ' class=CTitle>'
                        . '<a name="' . $anchor . '"></a>'
                        . $self->StringToHTML($topic->Title(), ADD_HIDDEN_BREAKS)
                    . '</' . $headerType . '>';
        ...
        if (defined $hierarchy || defined $summary ||
                defined $topic->Prototype() || defined $topic->Body())
            {
        ...
        if (defined $topic->Prototype())
            {
            $output .= $self->BuildPrototype($topic->Type(), $topic->Prototype(), $sourceFile, $file);
            };

        if (defined $topic->Body())
            {
            $output .= $self->NDMarkupToHTML($sourceFile, $topic->Body(), $topic->Symbol(),
...
#   if ($index + 2 >= scalar @$parsedFile)      #ND+, simple entries
#       {  return undef;  };
#
...
#       {  return undef;  };
    ...
    my $items = 0;                              #ND+, summary

    my $outputFile = $self->OutputFileOf($sourceFile);  #ND+, packaging
    ...
    . '<div class=Summary><div class=STitle>Summary</div>'
        ...
        while ($index < scalar @$parsedFile)
           ...
           my $target = NaturalDocs::SymbolTable->Lookup($topic->Symbol(), $sourceFile);
           my $targetFile = "";                 #ND+, package

           if (defined($target) && $target->File() ne $sourceFile)
                {
                $targetFile = $self->MakeRelativeURL($outputFile,
                                    $self->OutputFileOf($target->File()), 1);
                };

            $output .=
            '<a href="'. $targetFile . '#' .    #ND+, package
                $self->SymbolToHTMLSymbol($topic->Symbol()) . '" ' . $toolTipProperties . '>'
                . $self->StringToHTML($topic->Title(), ADD_HIDDEN_BREAKS)
            . '</a>';
...
sub BuildPrototype #(type, prototype, sourceFile, file)
    ...
    my ($self, $type, $prototype, $sourceFile, $file) = @_;
                                                ...
                                                #ND+, package/modeline
    my $language = (defined $file ? $file->Language() : NaturalDocs::Languages->LanguageOf($sourceFile));
    ...
    if ($file && $language->Name() eq 'Text File')
        ...
        if ($languageModeline)
            ...
            if ($t_language)                    #ND+, language override
                {
                $language = $t_language;
                }
            }
...
#       sourceFile - The <FileName> the target's defined in.
...
#
sub BuildToolTip #(symbol, sourceFile, type, prototype, summary)
    {
    my ($self, $symbol, $sourceFile, $type, $prototype, $summary) = @_;
    ...
    if (defined $prototype || defined $summary)
        ...
        if (!defined $number)
            ...
            if (defined $prototype)
                {                               #ND+, package/modeline
                $tooltipHTML .= $self->BuildPrototype($type, $prototype, $sourceFile, undef);
                };

            if (defined $summary)
                {
                # The fact that we don't have scope or using shouldn't matter because links shouldn't be included in the style anyway.
                $summary = $self->NDMarkupToHTML($sourceFile, $summary, undef, undef, $type, undef, NDMARKUPTOHTML_TOOLTIP);
                $tooltipHTML .= $summary;
                };
...
sub BuildClassHierarchy #(sourceFile, symbol)
    ...
    @children = sort { ::StringCompare($a, $b) } @children;
                                                ...
                                                #ND+, package
    # Dont generate a call hierarchary, if the class
    #
...
sub ClosingBrowserStyles
    {
    my ($self, $outputFile) = @_;               #ND+, syntax

    my $src = "";

    my @brushes = keys %imageSyntaxBrushes;

    if (defined $outputFile && scalar @brushes)
        {                                       #ND+, syntax 3+
        # Append the following:
        #
        #   To get SyntaxHighlighter to work on you page, you need to do the following:
        #
        #       o Add base files to your page: shCore.js and shCore.css
        #
        #       o Add brushes that you want (for example, shBrushJScript.js for
        #         JavaScript, see the list of all available brushes).
        #
        #       o Include shCore.css and shThemeDefault.css
        #
        #       o Create a code snippet with either <pre /> or <script /> method (see below)
        #
        #       o Call SyntaxHighlighter.all() JavaScript method
        #
        my $cssdir = $self->CSSDirectory();
        my $jsdir  = $self->JavaScriptDirectory();

        $src .= "<!-- Include required JS files, plus language brushes -->\n";

        foreach my $js ('shCore.js', @brushes)
             {
            $src .= '<script language="javascript" src="' .
                        $self->MakeRelativeURL($outputFile, NaturalDocs::File->JoinPaths($jsdir, $js), 1) .
                    '"></script>' . "\n";
            }

        $src .= "<!-- Include *at least* the core style and default theme -->\n";

        $src .= '<link href="' .
                    $self->MakeRelativeURL($outputFile, NaturalDocs::File->JoinPaths($cssdir, "shCore.css"), 1) . '"' .
                    ' rel="stylesheet" type="text/css"' . " />\n";

        $src .= '<link href="' .                # TODO, configuration
                    $self->MakeRelativeURL($outputFile, NaturalDocs::File->JoinPaths($cssdir, "shThemeDefault.css"), 1) . '"' .
                    ' rel="stylesheet" type="text/css"' . " />\n";

        my $jsrel = $self->MakeRelativeURL($outputFile, NaturalDocs::File->JoinPaths($jsdir, ""), 1);
        ...
        #   OLD interface
        #
        #   $src .= '<link type="text/css" rel="stylesheet" href="' .
        #               $self->MakeRelativeURL($outputFile, NaturalDocs::File->JoinPaths($cssdir, "SyntaxHighlighter.css"), 1) .
        #           '"></link>' . "\n";
        #
        #   foreach my $js ('shCore.js', @brushes)
        #       {
        ...
        #   $src .= "\t" . 'dp.SyntaxHighlighter.ClipboardSwf = \'' .
        #                       $self->MakeRelativeURL($outputFile, NaturalDocs::File->JoinPaths($jsdir, "clipboard.swf"), 1) . '\';' . "\n";
        #   $src .= "\t" . 'dp.SyntaxHighlighter.BloggerMode()' . "\n";
        #   $src .= "\t" . 'dp.SyntaxHighlighter.HighlightAll(\'SyntaxHighlighter\');' . "\n";
        #   $src .= '}' . "\n";
        #   $src .= '</script>' . "\n";
        #
...
#       sourceFile - The <FileName> the symbol is defined in.
...
#
sub BuildIndexLink #(string text, SymbolString symbol, SymbolString package, FileName sourceFile, TopicType type, string prototype, string summary, string style) => ( string, string )
    {
    my ($self, $text, $symbol, $package, $sourceFile, $type, $prototype, $summary, $style) = @_;

    $symbol = NaturalDocs::SymbolString->Join($package, $symbol);

    my $targetTooltipID = $self->BuildToolTip($symbol, $sourceFile, $type, $prototype, $summary);
    my $toolTipProperties = $self->BuildToolTipLinkProperties($targetTooltipID);

    my $indexHTML = '<a href="' . $self->MakeRelativeURL( $self->IndexDirectory(), $self->OutputFileOf($sourceFile) )
                                         . '#' . $self->SymbolToHTMLSymbol($symbol) . '" ' . $toolTipProperties . ' '
                                . 'class=' . $style . '>' . $text . '</a>';
    my $searchResultHTML = '<a href="' . $self->MakeRelativeURL( $self->SearchResultsDirectory(), $self->OutputFileOf($sourceFile) )
...
sub SymbolToHTMLSymbol #(symbol)
    ...
    $htmlSymbol =~ s/([^^A-Za-z0-9\-_.!~*\'()])/ sprintf "%%%x", ord $1 /eg;
    return $htmlSymbol;
    };
...
#       The text in HTML.
#
my %drawingOptions;                             #ND+, svg (yuk global must fix!)

sub NDMarkupToHTML #(sourceFile, text, symbol, package, type, using, style, imgSeq)
    {
    ...
    my $inDrawing;
                                                #ND+, quote
    my @splitText = split(/(<\/?(?:drawing|code|prefixcode|quote)[^>]*>)/, $text);
    ...
    while (scalar @splitText)
        ...
        if ($text =~ /^<code([^>]*)>$/)         #ND+
            ...
            #       $output .= "<pre name=\"SyntaxHighlighter\" class=\"$value:nogutter\">";
            #
            if ($highlightMode)
                {
                if ($options)
                    ...
                    foreach (@opts)
                        ...
                        if ($key eq 'hilite')   #ND+, syntax3
                            {
                            $value = lc($value);
                            $value =~ s/\s*//g;

                            foreach (@syntaxHiliteBrushes)
                                {
                                if (/[:,]\Q${value}\E(,|$)/)
                                    {
                                    my ($js, $brush) = split(/[:,]/);
                                    $imageSyntaxBrushes{$js} = 1;
                                    $output .= "<pre class=\"brush: $brush; toolbar: false;\">";
                                    $inCode = 2;
                                    last;
                                    };
                                }
                ...
                if (1 == $inCode && $highlightMode >= 2)
                    {                           #ND+, apply default brush
                    foreach (@syntaxHiliteBrushes)
                        {
                        if (/[:,]\Q${highlightLang}\E(,|$)/)
                            {
                            my ($js, $brush)= split(/[:,]/);
                            $imageSyntaxBrushes{$js} = 1;
                            $output .= "<pre class=\"brush: $brush; toolbar: false; gutter: false;\">";
                            $inCode = 2;
                            last;
                            };
                        }
        ...
        elsif ($text eq '<prefixcode>')
            {
            $output .= '<blockquote><pre class=CCode>';
            $inCode = 1;

            if ($highlightMode >= 2)            # highlight 'all' code sections
                {
                foreach (@syntaxHiliteBrushes)
                    {
                    if (/[:,]\Q${highlightLang}\E(,|$)/)
            ...
            {
            $output .= '<blockquote><pre class=CQuote>';
            $inCode = 1;
            }
            ...
            if ($options)
                ...
                foreach (@opts)
                    ...
                    $drawingOptions{$key} = $value
                        if ($key);
                    }
                }
                $inDrawing = 1;
            }
        elsif ($text eq '</drawing>')           #ND+, svg
            {
            undef %drawingOptions;
            $inDrawing = undef;
            }
        elsif ($inDrawing)                      #ND+, svg
            {
            my $imageSeq = ++${$imgSeqRef};     # increment file sequence
            my $outputFile = $self->OutputImageOf($sourceFile);

            $outputFile =~ s/(.*)\..*/\1/;
            $outputFile .= $imageSeq;
            $outputFile .= ".svg";
            $output .= NaturalDocs::Builder::toSVG::drawingToSVG($text, $outputFile, %drawingOptions);
            }
        else
            {
                ...
                if ($style == NDMARKUPTOHTML_GENERAL)
                    ...
                    foreach my $imageBlock (@imageBlocks)
                        ...
                        $imageBlock =~ s{<img mode=\"link\" target=\"([^\"]*)\" original=\"([^\"]*)\">}
                                                {$self->BuildImage($sourceFile, 'link', $1, $2, $imgSeqRef, \$imageContent)}ge;
            ...
            # ditaa images                      #ND+, ditaa
            $text =~ s/<ditaa([^>]+)>(.*?)<\/ditaa>/
                            $self->BuildDitaa($1, $2, $sourceFile, $imgSeqRef)/gse;

            # plantuml images                   #ND+, plantuml
            $text =~ s/<plantuml([^>]+)>(.*?)<\/plantuml>/
                            $self->BuildPlantUML($1, $2, $sourceFile, $imgSeqRef)/gse;

            # mscgen images                     #ND+, mscgen
            $text =~ s/<mscgen([^>]+)>(.*?)<\/mscgen>/
                            $self->BuildMscgen($1, $2, $sourceFile, $imgSeqRef)/gse;

            # sdedit images                     #ND+, sdedit
            ...
            if ($style == NDMARKUPTOHTML_GENERAL)
                ...
                $text =~ s{<img mode=\"inline\" target=\"([^\"]*)\" original=\"([^\"]*)\">}
                               {$self->BuildImage($sourceFile, 'inline', $1, $2, $imgSeqRef)}ge;
                }
            else
                {
            ...
            # ND+, Subheadings
            $text =~ s/<h2>/<h5 class=CSubHeading>/g;
            $text =~ s/<\/h2>/<\/h5>/g;

            # Admonition's
            $text =~ s/<admon-(.*)>/<div class=CAdmonition><div class=CAdmonition-$1>/g;
            $text =~ s/<\/admon-(.*)>/<\/div><\/div>/g;
            $text =~ s/<ah>/<h4 class=CHeading>/g;
            $text =~ s/<\/ah>/<\/h4>/g;

            # Bold <b>, italic <i>, underline <u>, strike-through <d>
            $text =~ s/<d>/<del>/g;
            $text =~ s/<\/d>/<\/del>/g;
            ...
            # ND+, Ordered lists
            $text =~ s/<ol>/<ol class=COrderedList>/g;
            $text =~ s/<ol type=([^>]+)>/\n\t\t<ol class=COrderedList type=$1>/g;

            # ND+, Indenting (bullet, description and ordered lists)
            #   CBulletIndent, CDescIndent and COrderedIndent
            #
            $text =~ s/<(\w*)Indent(\d+)>/<div class=C${1}Indent${2}>/g;
            $text =~ s/<\/(\w*)Indent(\d+)>/<\/div>/g;

            # Description lists (implemented as a table)
...
#   Function: BuildDitaa
#
#       Creates a HTML link to a ditaa image, invoking ditaa to generate the image from the
#       user ascii art and relating the generated file with the owning source file as an
#       auxiliary file.
#
#   Parameters:
#
...
sub BuildDitaa
    ...
    my $seq = ++${$imgSeqRef};                          # increment file sequence

    my ($vol, $dir, $file) = NaturalDocs::File->SplitPath($self->OutputFileOf($sourceFile, 1));

    my ($infile) = NaturalDocs::File->JoinPaths($vol.$dir, "ditaa-img-$seq.ditaa");
    my ($outfile) = NaturalDocs::File->JoinPaths($vol.$dir, "ditaa-out-$seq.log");

    $file = NaturalDocs::File->JoinPaths($vol.$dir, "$file-$seq.png");

    open(INFILE, '>' . $infile) or
        die "ND: cannot create Ditaa temporary file '$infile'\n";
    print INFILE $img;
    close(INFILE);
                                                        # TODO - configuration settings
    my $rv = system("java -jar ${Bin}/plugin/ditaa.jar $options $infile $file >$outfile");

    if ($rv)                                            # error, replace with code section
        {
        $img = NaturalDocs::NDMarkup->ConvertAmpChars($img);

        return '<blockquote><pre class="CDitaa">' . $img . '</pre></blockquote>';
        };

    unlink($infile);
    unlink($outfile);

    NaturalDocs::Project->RelateAuxiliary($sourceFile, $file, ::AUX_IMAGE());

    return '<img src='.$self->MakeRelativeURL($vol.$dir, $file).'>';
    };


#
#   Function: BuildPlantUML
#
#       Creates a HTML link to a PlantUML image, invoking PlantUML to generate the image from the
#       user ascii art and relating the generated file with the owning source file as an
#       auxiliary file.
#
#   Parameters:
#
#       options - User options for the PlantUML processor.
#       img - Image definition.
#       sourceFile - Source image that owns the link.
#
#   Returns:
...
sub BuildPlantUML
    ...
    my ($infile) = NaturalDocs::File->JoinPaths($vol.$dir, "plantuml-img-${seq}.plantuml");
    my ($outfile) = NaturalDocs::File->JoinPaths($vol.$dir, "plantuml-out-${seq}.log");

    $file = NaturalDocs::File->JoinPaths($vol.$dir, "$file-${seq}.png");

    open(INFILE, '>' . $infile) or
        die "ND: cannot create PlantUML temporary file '$infile'\n";
    print INFILE "\@startuml\n";
    print INFILE $img;
    print INFILE "\n\@enduml\n";
    close(INFILE);
    ...
    if (!exists $ENV{'GRAPHVIZ_DOT'})
        {                                               # resolve 'graphviz' path
        if (exists $ENV{'ProgramFiles'})
            {                                           # WIN32 installations
            my $ProgramFiles = $ENV{'ProgramFiles'};
            my @versions = ('2.38', '2.36', '2.34');

            foreach my $version (@versions)
                {
                my $graphvizdir = "${ProgramFiles}/Graphviz${version}";
                if (-d $graphvizdir)
                    {
                    if (-x "${graphvizdir}/bin/dot.exe")
                        { $dot = "${graphvizdir}/bin/dot.exe"; }
                    elsif (-x "${graphvizdir}/dot.exe")
                        { $dot = "${graphvizdir}/dot.exe"; }
                    last;
                    }
                }
            $dot =~ s/\\/\//g
        ...
        else                                            # UNIX installations
            {
            my @paths = ('/usr/local/bin/graphviz/dot', '/usr/local/bin/dot', '/usr/bin/dot');
        ...
        $dot = "-graphvizdot \"$dot\"" if ($dot);
        }

                                                        # TODO - configuration settings
    my $rv = system("java -jar ${Bin}/plugin/plantuml.jar -pipe ${dot} -tpng ${options} <${infile} >${file} 2>${outfile}");

    if ($rv)                                            # error, replace with code section
        {
        $img = NaturalDocs::NDMarkup->ConvertAmpChars($img);

        return '<blockquote><pre class="CPlantUML">' . $img . '</pre></blockquote>';
        };
...
#
#       Creates a HTML link to a mscgen image, invoking mscgen to generate the image from the
#       user ascii art and relating the generated file with the owning source file as an
#       auxiliary file.
#
#   Parameters:
...
sub BuildMscgen
    ...
    my ($infile) = NaturalDocs::File->JoinPaths($vol.$dir, "mscgen-img-$seq.msc");
    my ($outfile) = NaturalDocs::File->JoinPaths($vol.$dir, "mscgen-out-$seq.log");

    $file = NaturalDocs::File->JoinPaths($vol.$dir, "$file-$seq.png");

    open(INFILE, '>' . $infile) or
        die "ND: cannot create Mscgen temporary file '$infile'\n";
    print INFILE "msc {\n";
    print INFILE $img;
    print INFILE "}";
    close(INFILE);
                                                        # TODO - configuration settings
    my $rv = system("${Bin}/plugin/mscgen -T png -i $infile -o $file $options >$outfile");

    if ($rv)                                            # error, replace with code section
        {
        $img = NaturalDocs::NDMarkup->ConvertAmpChars($img);

        return '<blockquote><pre class="CMscgen">' . $img . '</pre></blockquote>';
        };
...
#
#       Creates a HTML link to a sdedit image, invoking mscgen to generate the image from the
#       user ascii art and relating the generated file with the owning source file as an
#       auxiliary file.
#
#   Parameters:
...
sub BuildSeqDiagEditor
    ...
    my ($infile) = NaturalDocs::File->JoinPaths($vol.$dir, "sdedit-img-$seq.sd");
    my ($outfile) = NaturalDocs::File->JoinPaths($vol.$dir, "sdedit-out-$seq.log");

    $file = NaturalDocs::File->JoinPaths($vol.$dir, "$file-$seq.png");

    open(INFILE, '>' . $infile) or
        die "ND: cannot create sdedit temporary file '$infile'\n";
    print INFILE $img;
    close(INFILE);
                                                        # TODO - configuration settings
    my $rv = system("java -jar ${Bin}/plugin/sdedit.jar -o $file -t png -r Portrait $infile >$outfile 2>&1");

    if ($rv)                                            # error, replace with code section
        {
        $img = NaturalDocs::NDMarkup->ConvertAmpChars($img);

        return '<blockquote><pre class="CSdEdit">' . $img . '</pre></blockquote>';
        };
...
#   Parameters:
#
#       tableid - Table (object() indentifier.
...
sub BuildTable
    ...
    my $ident = "\t" x ($embedded ? 4 : 1);
    my $output = "\n";

    $output .=
        $ident.'<div class="CTable">' .
                '<a name="' . $self->SymbolToHTMLSymbol($table->Title()) . '"></a>' . "\n" .
        "$ident<table border=0 cellspacing=0 cellpadding=0 class=TBody>\n";

    # Caption
    if (defined (my $caption = $table->Caption()))
        {
        $output .= "$ident<caption class=TCaption>".$caption."</caption>\n";
        }
    ...
    for (my $row = 1; $row <= $rows; $row++)
        ...
        # Head or data
        # SubHeads must be contained within the body
        if ($row == 1)
            {
        ...
        $output .=
            "$ident\t". ($ishead ? '<thead>' : '<tbody>') . "<tr class=TRow>\n";
        ...
        for (my $col = 1; $col <= $cols; $col++)
            ...
            # Spanned/missing cell are undef
            next if (! defined $cell);

            my $spanning = $cell->Spanning ();
            my $width = $cell->Width ();

            # Definition
            $output .= "$ident\t\t";
            $output .= '<' . ($cell->IsHead() ? 'th' : 'td');
            $output .= ' colspan="' . $spanning . '"' if ($spanning > 1);
            $output .= ' width="' . $width . '%"' if ($width > 0);
            if ($cell->IsNull())
                {
                $output .= ' class=TNull';
                }
                ...
                {
                $output .= ' class=' . ($cell->IsHead() ? "THead" : "TNormal");
                $output .= ' class=' . "TColumn$col";
                }
            $output .= '>';

            # Content (ignore null columns)
            if (! $cell->IsNull())
                {
                my $content = $cell->ContentGet(1);

                if (defined $content && length ($content))
                    {                                   # expand any embedded tables
                    $content =~ s/<table=([^>]+)>/$self->BuildTable($1, $sourceFile)/ge;
                    $output .= $content;
                    }
                else
                    {
        ...
        $output .= "$ident\t". ($ishead ? '</thead>' : '</tbody>') . "</tr>\n";
        }
    ...
    # Title
    $output .= "<div class=TTitle>" . $table->Title() . "</div>"
...
#   Returns:
#
#       The result in HTML.  If the mode was "link", the target image's HTML is added to <imageContentRef>.
#
sub BuildImage #(sourceFile, mode, target, original, imgSeqRef)
    ...
    if ($image)
        ...
        if ($mode eq 'inline')
            ...
            my $output =
            '<a href="#Image' . $imageSeq . '" class=CImageLink>'
