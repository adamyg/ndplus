...
#
#   Function: DetermineIfImage
#
#       Detemines whether a known image type.
#
sub DetermineIfImage #(filename)
    {                                                   #ND+, package
    my ($self, $imageFile) = @_;
    my $extension = lc( NaturalDocs::File->ExtensionOf($imageFile) );

    return ((defined $extension && exists $imageFileExtensions{$extension}) ? 1 : 0);
    };
...
sub ParseSupported
    ...
    #           under low memory conditions; this is a an issue today?
    #
    #   Try running
